...
import { IndianRupee, Briefcase, Activity, TrendingDown } from 'lucide-react';
import {
...
const MOCK_REVENUE_DAILY = [
  { month: '1 Jun', Rental: 3200, Piling: 2100, 'O&M': 1200 },
  { month: '8 Jun', Rental: 4100, Piling: 2800, 'O&M': 1500 },
  { month: '15 Jun', Rental: 3800, Piling: 2500, 'O&M': 1800 },
  { month: '22 Jun', Rental: 4500, Piling: 3000, 'O&M': 2000 },
  { month: '29 Jun', Rental: 4200, Piling: 2700, 'O&M': 1600 },
];
...
const MOCK_REVENUE_YEARLY = [
  { month: '2021', Rental: 520000, Piling: 310000, 'O&M': 180000 },
  { month: '2022', Rental: 680000, Piling: 420000, 'O&M': 240000 },
  { month: '2023', Rental: 810000, Piling: 490000, 'O&M': 290000 },
  { month: '2024', Rental: 950000, Piling: 570000, 'O&M': 350000 },
];

const MOCK_REVENUE_TREND = MOCK_REVENUE_MONTHLY;
...
const AdminDashboard = () => {
  ...
  const MOCK_BY_PERIOD = {
    ...
    monthly: MOCK_REVENUE_MONTHLY,
    yearly: MOCK_REVENUE_YEARLY,
  };
  ...
  useEffect(() => {
    const fetchTrend = async () => {
      try {
        const [trendRes, pieRes] = await Promise.allSettled([
          api.get('/admin-dashboard/charts/revenue-line', { params: { period: trendPeriod } }),
          api.get('/admin-dashboard/charts/revenue-pie', { params: { period: trendPeriod } }),
        ]);
        if (trendRes.status === 'fulfilled' && trendRes.value.data?.data?.length) {
          setRevenueTrend(trendRes.value.data.data);
        } else {
          setRevenueTrend(MOCK_BY_PERIOD[trendPeriod] || MOCK_REVENUE_MONTHLY);
        }
        if (pieRes.status === 'fulfilled' && pieRes.value.data?.data?.length) {
          setRevenuePie(pieRes.value.data.data.map(d => ({ name: d.vertical, value: d.revenue })));
        }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
        <div>
          <h1 className="text-xl md:text-2xl font-bold text-white">Company Overview</h1>
          <p className="text-xs md:text-sm text-gray-500 mt-0.5">Cross-vertical operational summary</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <MetricCard title="Total Revenue (YTD)" value={`\u20B9${Number(data.total_revenue).toLocaleString()}`} icon={IndianRupee} trend={12.5} trendLabel="vs last year" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <div className="bg-surface border border-border rounded-lg p-4 md:p-6 shadow-sm h-full flex flex-col">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4 md:mb-6">
                <h3 className="text-sm md:text-lg font-medium text-white">Revenue Trend</h3>
                <div className="flex gap-1 bg-background rounded-md p-0.5">
                  {['daily', 'monthly', 'yearly'].map((p) => (
                    <button
                      key={p}
                      onClick={() => setTrendPeriod(p)}
                      className={`px-2 md:px-2.5 py-1 rounded text-[10px] md:text-xs font-medium capitalize transition-colors ${trendPeriod === p ? 'bg-primary text-white' : 'text-gray-400 hover:text-white'}`}
                    >
                      {p}
                    </button>
                  ))}
